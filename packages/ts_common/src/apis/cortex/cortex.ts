...
import { DEFAULT_SANDBOX_TIMEOUT } from './sandbox_interface'
...
`
	...
	*/
	...
	const feedback = this.utilities.sounds || {
	    error: () => {},
	    activated: () => {},
	    ok: () => {},
	    success: () => {}
	...
	aux_parameters.get_embedding = this.utilities.get_embedding || (async () => { throw new Error('Embedding function not configured') }); 	
