...
let log = tsc.logger.get_logger({id :'evmw'})
 ...
 */
export async function get_smart_wallet_by_address(addr : string, p : ethers.providers.JsonRpcProvider , tx_type : tsc.web3.smart_wallet.TxType) {
    check_reqs() ;
    let w = ( wallet_cache[addr] || await get_wallet_by_address(addr) ) ;
    wallet_cache[addr]  = w ; 
    let ops = { privateKey : w.privateKey , provider : p , tx_type}  ; 
    let sw = new tsc.web3.smart_wallet.SmartWallet(ops) ;
    await sw.init() ;
    return sw; 
} 
