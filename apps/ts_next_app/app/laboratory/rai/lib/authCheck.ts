 ...
 *
 * When a user was previously logged in with cloud storage and then logs out
 * (or session expires), surreal_query sends unauthenticated requests that
 * return empty results rather than errors. This module detects that state
 * and notifies the user with actionable options.
 */

import { getRaiStore } from './storage';
 ...
 * On success, reloads RAI data from cloud so the UI updates in place.
...
async function signInAndReload(
  ...
  try {
    ...
    if (window.__rai__?.dispatch) {
      await window.__rai__.dispatch('loadSettings');
      await window.__rai__.dispatch('loadTemplates');
      await window.__rai__.dispatch('loadDotPhrases');
    }
 ...
 * Uses the same pattern as InsightsContext.tsx:50-55.
 */
...
function authToastButtons(
  ...
  return [
    ...
    React.createElement('button', {
      key: 'local',
      onClick: () => {
        if (typeof window !== 'undefined' && window.__rai__?.dispatch) {
          window.__rai__.dispatch('switchStorageMode', 'local');
        }
        if (emitFn) {
...
// ─── Template-specific fallback toast (not debounced — direct response to user action) ─

/**
 * Notify user that their template was saved to local storage only because
 * they're in cloud mode but not authenticated.
 */
export function notifyTemplateSavedLocally(
  emitFn?: (type: string, payload: Record<string, any>) => void,
): void {
  if (emitFn) {
    emitFn('cloud_auth_required', { context: 'save_templates', action: 'local_fallback' });
  }
    ...
    render: ({ onClose }: { onClose: () => void }) => {
      return React.createElement('div', {
        ...
        React.createElement('div', {
          ...
          'Your template was saved to the current browser only (not the cloud). Log in to access cloud templates (syncs across devices), or switch to local storage to use RAI without logging in (templates will not sync to other devices).'
        ),
        React.createElement('div', { style: { display: 'flex', gap: '8px' } },
          ...authToastButtons(emitFn, onClose),
        ),
