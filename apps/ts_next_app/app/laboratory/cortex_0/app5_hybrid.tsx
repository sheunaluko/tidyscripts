...
const  Component: NextPage = (props : any) => {
    ...
    // TTS with interruption hook
    const tts = useTTSWithInterruption({
	onInterrupt: () => {
	    log('TTS interrupted by user speech');
	},
	onError: (error: Error) => {
	    log(`TTS error: ${error.message}`);
	},
	positiveSpeechThreshold: 0.9, // Higher = less sensitive (avoid TTS self-trigger)
	negativeSpeechThreshold: 0.75,
	minSpeechMs: 600, // Longer duration to avoid false positives
	verbose: false,
    });
    ...
    */
    ...
    useEffect( ()=> {
        let speak = async function(content : string) {
            await tts.speak(content, playbackRate);
        }
