...
import { useInterruptibleVoice } from '../components/voice_interface/lib';
...
const  Component: NextPage = (props : any) => {
    ...
    // Removed: transcribe and transcribeRef - now using voice.isListening from hook
    ...
    // Removed: transcription_similarity_threshold - echo cancellation handled by hook
    // Removed: interim_result - now using voice.interimResult from hook
    ...
    // Voice hook with VAD + WebSpeech (note: transcription_cb is defined later)
    const voice = useInterruptibleVoice({
      onTranscription: (text: string) => {
        // Call transcription_cb which is defined below
        transcription_cb(text);
      },
      onInterrupt: () => {
        log('Voice interrupted by user speech');
      ...
      language: 'en-US',
      // Default VAD parameters (tested and working)
    ...
    */
    ...
    // Removed: Event listener for interim results - now using voice.interimResult from hook
    ...
    }, [chat_history, thought_history, log_history, last_ai_message, voice.isListening, playbackRate, focusedWidget, voice.interimResult]);    
    ...
    // Removed: useEffect for interim_result "stop" detection - hook handles auto-interruption via VAD
	...
	// Removed: listen_while_speaking check - echo cancellation handled by voice hook
	// Removed: String similarity comparison - no longer needed
	// Removed: Manual "stop" detection - hook handles auto-interruption via VAD

	// Route transcript to cortex or active function
    ...
    // Removed: handleSwitch - now using inline onChange with voice.startListening/stopListening
		...
		<Button variant='outlined' style={{width:"10%" , borderRadius : "20px", marginLeft : '25px' , marginTop : '11px'}}
			onClick={function() {
				...
				if (!plots_initialized) {
				    ...
				    on_init_audio() // Initialize mic for visualization only
				} else {
				    GLOBAL_PAUSE = false
				} 
			    } else {
	...
	<Box style={{width : "60%" }}>
		...
		<AccordionDetails>
			...
			function() {
				...
				vi.tts.cancel_speech();
				// Don't stop listening - that's controlled by Listen toggle
				add_user_message(`I no longer wanted to listen to your output and so I interrupted your speech. Do not respond until I prompt you again`);
			    }
...
async function on_init_audio() {
    ...
    //start streaming microphone data to the mic graph (for Bokeh visualization only)
    log(`Initializing microphone for visualization`) ;
    await wa.initialize_microphone() ;

    //register graph callback for Bokeh visualization
    ...
    // Removed: vi.initialize_recognition() - now handled by voice hook
    // Removed: Event listener for final results - now using voice.onTranscription callback
