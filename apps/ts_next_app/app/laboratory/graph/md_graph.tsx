  ...
  useEffect(() => {
    ...
    graph.forEachEdge((edge, attributes) => {
      ...
      if (typeof parallelIndex === 'number' && parallelMaxIndex > 0) {
        // Use smaller curvature for better visual appeal
        ...
        // Single edges use straight with arrows if directed
...
export const GraphComponent: FC<GraphComponentProps> = ({
  ...
  // Generate a stable key based on the data content to force re-render when data changes
  const graphKey = useMemo(() => {
    const nodeIds = data.nodes.map(n => n.id).sort().join(',');
    const edgeIds = data.edges.map(e => `${e.source}-${e.target}`).sort().join(',');
    return `${data.nodes.length}-${data.edges.length}-${nodeIds.slice(0, 50)}-${edgeIds.slice(0, 50)}`;
  }, [data]);
