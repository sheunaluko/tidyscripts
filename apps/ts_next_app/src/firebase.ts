...
export function give_feedback(msg : string) {
    ...
    store_data(db, ["feedback"] , user_data ) ; 
} 
 ...
 * @returns A promise that resolves when the document is successfully written.
 */
const store_data = async (db: Firestore, path: string[], data: UserData): Promise<void> => {
    try {
	// @ts-expect-error
    const collection_ref = collection(db, ...path);
    await addDoc(collection_ref, data);
    console.log(`Document at path ${path.join('/')} has been added successfully.`);
  } catch (error) {
