...
function get_uid_token( uid : string) {  return UID_TOKEN_MAP[uid] }
function set_uid_token( uid : string, token : string ) {  UID_TOKEN_MAP[uid] = token }
...
async function get_surreal() {
    ...
    let username = process.env['SURREAL_TIDYSCRIPTS_BACKEND_USER'] as string
    let password = process.env['SURREAL_TIDYSCRIPTS_BACKEND_PASSWORD'] as string
    let url      = process.env['SURREAL_TIDYSCRIPTS_BACKEND_URL'] as string        
...
async function user_exists(user_id : string, email : string) {
    let db = await get_surreal() ;
    let query = `select * from user where user_id = $user_id and email = $email`  ;
    let vars = { user_id, email }
    logger.info(`Checking for user with : userid,email = ${vars.user_id},${vars.email}`)  ;
    let result : any = await db.query(query,vars)  ;
...
export const surrealQuery = onCall(
  async (request: CallableRequest<any>) => {
      ...
      if (!request.auth) {
	  let error = "Unauthenticated request. User must be logged in." ;
	  return {
	      success: false ,
	      error 
	  }
      ...
      let email = request.auth.token.email as string;

      // --
      var REQUEST_TOKEN : string  = "" ;
      var REQUEST_SUMMARY : string = "" ; 

      //check if there is a user token cached already
      let token  = get_uid_token(uid) ;
      if (token) {
	  ...
	  if (is_token_valid(token))  {
	      ...
	      // - USER SIGN IN 
	      let result = await sign_in_up_user(ops)  ;
	      if (!result.success )  {
	  ...
	  let the_user_exists = await user_exists(uid, email) ; 
	  if (the_user_exists) {
...
export const newUserTable = onCall(
  async (request: CallableRequest<any>) => {
	  ...
	  let new_name  = request.data.name.replace(/\s/g,"_") ;
	  logger.info(`replacing name with ${new_name}`)

	  
	  let db = await get_surreal() ;
	  let query = template_user_table.replace(/@TABLE_NAME/g, new_name) ; 
