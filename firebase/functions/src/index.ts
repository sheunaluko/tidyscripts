...
import  * as tsn from "tidyscripts_node" ; //wow :) 

const common = tsn.common ; 
...
//TODO :
//For now, just forget about using tidyscripts apis in the cloud function
//later can read this https://github.com/vercel/turborepo/issues/1121
//JUST compute eid client side and pass it with the EmbeddingData 
...
export const storeEmbedding = onCall(
  async (request: CallableRequest<EmbeddingData>) => {
    ...
    if (!metadata.sid) {
      ...
      logger.info(Object.keys(common))
      logger.info(Object.keys(common.apis))      

     let eid = await common.apis.cryptography.object_sha256(request.data) as string; 
