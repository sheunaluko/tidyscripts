...
export function get_transcript(url : string) {
    ...
    if (!passed ) {  return }
    let d  = path.join(tidyscripts_data_dir as string, 'tmp' )    
    let transcript = extractAndCleanSubtitles(url, d) ;
    return transcript ; 
 ...
 */
async function extractAndCleanSubtitles(youtubeUrl : string, tmpWorkingDirectory : string): Promise<string | undefined> {
    try {
